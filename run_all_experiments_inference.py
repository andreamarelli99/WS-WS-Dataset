...
def get_dataloader(config):
    ...
    train_dataset = seruso_datasets.Seruso_three_classes_flow(img_root = dataset_dir, flow_root = flow_dir, classes_subfolders = classes_subfolders, return_img_path = True, dstype = 'training', transform = test_transform, augment = False)
    val_dataset = seruso_datasets.Seruso_three_classes_flow(img_root = dataset_dir, flow_root = flow_dir, classes_subfolders = classes_subfolders, return_img_path = True, dstype = 'validation', transform = test_transform, augment = False)
...
def main():
    ...
    datasets, class_names = get_dataloader(config)
    ...
    for dataset in datasets:
        if config['model'] == 'GradCAM' or config['model'] == 'GradCAMPlusPlus' or config['model'] == 'LayerCAM':
            model = Std_classifier_inference(config, dataset, sam_enhance=sam_enhance, method = config['model'])
        elif config['model'] == 'PuzzleCAM':
            model = Puzzle_CAM_inference(config, dataset, sam_enhance=sam_enhance)
        elif config['model'] == 'POF_CAM':
            model = POF_CAM_inference(config, dataset, sam_enhance=sam_enhance)
        else:
            raise ValueError("Invalid model type. Choose from 'GradCAM', 'PuzzleCAM', or 'POF_CAM'.")
        
        model.make_all_cams(save_mask=save_mask, visualize=visualize, norm=True, max_item=10000)
