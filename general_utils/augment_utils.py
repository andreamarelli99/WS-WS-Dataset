...
class Three_images_trasform:
    def __init__(self, params):
        self.horizontal_flips = params[0]
        self.vertical_flips = params[1]
        self.rotation_degrees = params[2]

    
    def __call__(self, images, revert = False):

        for i in range (3):
            if bool(self.horizontal_flips[i]):
                images[i] = F.hflip(images[i])
            if bool(self.vertical_flips[i]):
                images[i] = F.vflip(images[i])
            if revert:
                images[i] =  F.rotate(images[i], -self.rotation_degrees[i])
            else:
                images[i] =  F.rotate(images[i], self.rotation_degrees[i])
...
class Three_images_batch_trasform:
    def __init__(self, params):
        self.horizontal_flips = params[0] # list of 3 torch.Size([8])
        self.vertical_flips = params[1] # list of 3 torch.Size([8])
        self.rotation_degrees = params[2] # list of 3 torch.Size([8])

    def __call__(self, images, revert=False):
        transformed_images = []
        for i in range(3):
            batch_transformed_images = []
            for batch_idx in range(images[0].size(0)):
            
                img = images[i][batch_idx]  # Select image from batch

                if revert:
                    img = F.rotate(img, -int(self.rotation_degrees[i][batch_idx]))
                else:
                    img = F.rotate(img, int(self.rotation_degrees[i][batch_idx]))

                if bool(self.vertical_flips[i][batch_idx]):
                    img = F.vflip(img)

                if bool(self.horizontal_flips[i][batch_idx]):
                    img = F.hflip(img)
                
                batch_transformed_images.append(img)

            transformed_images.append(torch.stack(batch_transformed_images))
...
class Denormalize:
    ...
    def __call__(self, image):
        image = np.asarray(image)
        denorm_image = np.empty_like(image, np.float32)

        denorm_image[..., 0] = ((image[..., 0]*self.std[0]) + self.mean[0] ) #* 255.
        denorm_image[..., 1] = ((image[..., 1]*self.std[1]) + self.mean[1] ) #* 255.
        denorm_image[..., 2] = ((image[..., 2]*self.std[2]) + self.mean[2] ) #* 255.
